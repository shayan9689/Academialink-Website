...
export default function NotificationsPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/papers')
      ...
      .then((data: { id: string; title: string; created_at?: string; submitted?: string }[]) => {
        setActivities(
          (Array.isArray(data) ? data : []).map((p) => ({
            id: p.id,
            text: `You submitted "${p.title.length > 60 ? `${p.title.slice(0, 60)}…` : p.title}"`,
          ...
          <div className={layoutStyles.dashboardBody}>
            ...
            <main className={layoutStyles.main}>
              ...
              <div className={styles.content}>
                ...
                <p className={styles.subtitle}>Your recent activity and submissions.</p>
                {loading ? (
                  <p className={styles.text} style={{ color: '#94a3b8' }}>Loading…</p>
                ) : activities.length === 0 ? (
                  <p className={styles.text} style={{ color: '#94a3b8' }}>No activity yet. Submit a paper to see it here.</p>
                ) : (
